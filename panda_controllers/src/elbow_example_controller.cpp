...
#include <panda_controllers/elbow_example_controller.h>
    ...
    for (size_t i = 0; i < q_start.size(); i++) {
      if (std::abs(state_handle.getRobotState().q_d[i] - q_start[i]) > 0.1) {
        ROS_ERROR_STREAM(
            ...
            "running this example. Run `roslaunch panda_controllers move_to_start.launch "
...
PLUGINLIB_EXPORT_CLASS(panda_controllers::ElbowExampleController,
